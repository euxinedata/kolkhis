...
- Catalog API (proves rename landed in DuckLake metadata)
...
class TestRenameView:
    ...
    def test_rename_nonexistent_view_fails(self, api):
        status, job, _ = submit_and_wait(
            api,
            f"ALTER VIEW development.{SCHEMA}.e2e_no_such_view RENAME TO x",
        )
...
class TestRenameTable:
    ...
    def test_rename_table(self, api):
        ...
        # Create table via dbt session (lands in DuckLake)
        result = dbt_query(
            api, self.session_id,
        ...
        # Verify new name in DuckLake catalog
        resp = api.get(f"/api/catalog/databases/development/schemas/{SCHEMA}/objects")
        resp.raise_for_status()
        names = [o["name"] for o in resp.json()["objects"]]
    ...
    def test_rename_nonexistent_table_fails(self, api):
        status, job, _ = submit_and_wait(
            api,
            f"ALTER TABLE development.{SCHEMA}.e2e_no_such_tbl RENAME TO x",
        )
        assert status == "failed"


# ---------------------------------------------------------------------------
# RENAME SCHEMA — DuckLake does not support ALTER SCHEMA RENAME
# ---------------------------------------------------------------------------
@pytest.mark.skip(reason="DuckLake does not support ALTER SCHEMA RENAME")
class TestRenameSchema:
    """ALTER SCHEMA db.old_name RENAME TO new_name — not supported by DuckLake."""
...
class TestRenameDatabase:
    ...
    Database rename modifies OrgDatabase record + renames PG metadata schema.
    """
